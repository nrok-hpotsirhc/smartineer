...
- **Keine Binär-Bilder** (PNG/JPG) — App-Icons sind SVG (Performance + git-friendly). **Ausnahme:** Das Brand-Logo (`icons/smartineer-logo.png`) ist eine vorgegebene 3D-Illustration und liegt als optimiertes PNG mit transparentem Hintergrund vor. Es wird seit v60 breit genutzt: Topbar, PWA-Install-Prompt, Favicon (`<link rel="icon">`), Apple-Touch-Icon (`<link rel="apple-touch-icon">`), Open-Graph-/Twitter-Share-Bild sowie als zusaetzlicher PNG-Eintrag im Manifest. Seit v61 ist das Brand-PNG zugleich die **primaere App-Kachel (Home-Screen-Icon)**: im Manifest steht es als erstes Icon mit `purpose: "any maskable"` in den Groessen `any`, `192x192` und `512x512`; `apple-touch-icon` wird zusaetzlich mit den Groessen 192 und 512 deklariert; Windows-Pin nutzt `msapplication-TileImage` / `msapplication-TileColor`. Innerhalb der App wird es als Hero-Logo in den Stage-Headern Dashboard, Cheatsheets, Schulungen-Index und Schueler-Index gerendert. Die SVG-Icons (`icons/icon.svg`, `icon-192.svg`, `icon-512.svg`) bleiben als zusaetzliche Manifest-Eintraege fuer SVG-faehige Plattformen erhalten. Eine vektorisierte, vereinfachte Variante des Brand-Logos liegt zur freien Verwendung in `logo/smartineer-logo-flat.svg`.
...
## 17. Schüler-Bereich (Mathematik Klasse 1–10, Deutsch/Naturwissenschaften/Geschichte/Sprachen Klasse 5–10)
...
```js
{
    ...
    subjects: { mathe: { label: 'Mathematik' }, deutsch: { label: 'Deutsch' }, englisch: { label: 'Englisch' }, franzoesisch: { label: 'Franzoesisch' }, latein: { label: 'Latein' } },
    content: {
...
- **Sofort-Feedback im Quiz (P-UI-QUIZ-INSTANT-FEEDBACK, seit v111):** `Antwort pruefen` wertet unmittelbar aus und haelt an, statt zur naechsten Aufgabe zu springen. Angezeigt werden `Richtig` / `Leider falsch`, die eigene Antwort und bei falscher Antwort die **korrekte Antwort**. Formel/Merksatz und Musterloesung erscheinen darunter in einem `<details>`-Block: bei falscher Antwort **aufgeklappt** (Lernmoment), bei richtiger Antwort **eingeklappt** (Quizfluss). Weiter geht es ausschliesslich ueber `Naechste Aufgabe` bzw. `Auswertung anzeigen` — kein Auto-Advance. Der Fortschrittsbalken zaehlt beantwortete Aufgaben, nicht den Index. Die Endauswertung bleibt unveraendert erhalten.
- **Keine Korrektur nach dem Pruefen.** Sobald das Feedback steht, ist die Antwort gewertet und in `smartineer_schueler_progress_v1` geschrieben; das Eingabefeld verschwindet.
- **Training fuer alle Mittelstufen-Faecher (Mathe, Deutsch, Physik, Chemie, Biologie, Geschichte, Englisch, Franzoesisch, Latein):** Fachkarten zeigen getrennt `Training oeffnen` und `10-Fragen-Quiz`. Sprachkarten (Englisch/Franzoesisch/Latein) zeigen zusaetzlich einklappbare Abschnitte `Zahlen`, `Vokabeln`, `Grammatik` mit eigenen Training-/Quiz-Starts. Training zeigt genau eine Aufgabe, Formel/Merksatz (`f`) und optional Musterloesung (`s`), plus `Als geloest markieren` mit Persistenz in `smartineer_schueler_progress_v1`. **Formel/Merksatz ist im Training per Default eingeklappt** (P-UI-SCHUELER-FORMEL-TOGGLE, seit v71): ein Header-Button `Tipp anzeigen` / `Ausblenden` steuert die Sichtbarkeit; bei eingeklapptem Zustand erscheint der Hinweis `Versuche es zuerst selbst. Bei Bedarf den Tipp einblenden.` Beim Wechsel der Aufgabe und beim Eintritt in Training/Quiz wird der Toggle auf `false` zurueckgesetzt.
- **Fortschrittsanzeige in allen Stages (P-UI-SCHUELER-PROGRESS, seit v109):** Die Klassen-Stage zeigt eine Gesamtkarte (Ring + Balken + `X von Y Aufgaben geloest`) und pro Klassenkarte Prozentwert, Balken und `geloest / gesamt` in der Klassenfarbe (`SCHUELER_CLASS_COLORS`, 1:1 auf `k1..k10` gemappt). Die Faecher-Stage zeigt eine Klassen-Zusammenfassung sowie je Fachkarte Prozentbadge, Balken und `geloest / gesamt · offen`. Abschnitte tragen `geloest / gesamt`, einen Mini-Balken und bei 100 % das Badge `fertig`. Training zeigt zusaetzlich zum Batch-Fortschritt eine Zeile `Fach gesamt` und den Sprungbutton `Naechste offene`; der Quiz-Result-Screen zeigt `n Aufgaben neu freigeschaltet` und den aktuellen Fachfortschritt.
- **`Nur offene`-Start (seit v109):** Fach- und Abschnittskarten bieten neben `Training oeffnen` einen Start, der bereits geloeste Items herausfiltert. Sind alle Items geloest, faellt die Auswahl auf den vollen Pool zurueck (Wiederholung bleibt moeglich).
...
| 8      | pool     | **Mathematik/Deutsch/Physik/Chemie/Biologie/Geschichte je 200 Items.** Sprache: Englisch, Franzoesisch, Latein je 100 Zahlen-, 100 Vokabel- und 200 Grammatikfragen. Themen: LGS/Pythagoras/Potenzen, Eroerterung/Drama/Medienkritik/Sprachebenen, Elektrik/Atombau/Verdauung/Kreislauf, Revolution/Industrialisierung, A2/B1-Sprachprogression. |
| 9      | pool     | **Mathematik/Deutsch/Physik/Chemie/Biologie/Geschichte je 200 Items.** Sprache: Englisch, Franzoesisch, Latein je 100 Zahlen-, 100 Vokabel- und 200 Grammatikfragen. Themen: Quadratik/Trigonometrie/Stochastik, Bewerbung/Novelle/Sprachvarietaeten/Desinformation, Energie/Druck/Saeure/Base/Genetik/Evolution, Kaiserreich/WK1/Weimar/NS, B1-Grammatik/lateinische Nebensaetze. |
| 10     | pool     | **Mathematik/Deutsch/Physik/Chemie/Biologie/Geschichte je 200 Items.** Sprache: Englisch, Franzoesisch, Latein je 100 Zahlen-, 100 Vokabel- und 200 Grammatikfragen. Themen: Exponentialfunktionen/Koerper/Trigonometrie, materialgestuetztes Schreiben/Textanalyse/Gedicht/Drama/Medienkritik, Radioaktivitaet/Optik/Organik/Molekularbiologie/Biodiversitaet, WK2/Shoah/Kalter Krieg/Wiedervereinigung/Globalisierung, B1-Konsolidierung/lateinische Stilistik. |

**Naturwissenschaften-Pools (Klasse 5–10, seit v65; auf 200 je Klasse/Fach seit v73):** Pro (Klasse, Fach) ein Pool mit `{q, a, f, s}`-Schema; aktuell 18 Pools × 200 = 3.600 NW-Aufgaben. Themen folgen dem **NRW-Kernlehrplan Sekundarstufe I** (KLP NW SI fuer Klasse 5/6 integrierter Naturwissenschaften-Unterricht; KLP Physik/Chemie/Biologie SI fuer Klasse 7–10). `q` und `a` bleiben Plain-Text/KaTeX; `f` und `s` duerfen kuratiertes HTML + KaTeX fuer Formel/Merksatz und Musterloesung enthalten, aber niemals User-Input. Antworten sind kurze Strings (Zahl, Begriff, Summenformel) und werden ueber `normalize()` (trim, whitespace weg, Komma->Punkt, Kleinschreibung, seit v73 zusaetzlich Akzent-/Apostroph-Toleranz fuer Sprachen) verglichen. Erweiterungen erfolgen bevorzugt append-only ueber `js/data/schueler_200_topups.js`; Stable-QID und Schueler-Progress entstehen aus Stem+Antwort, nicht aus Ingenieurs-Indizes.

**Mathematik-Pools (Klasse 5–10, seit v70; auf 200 je Klasse seit v73):** Pro Klassenstufe ein 200er Pool mit `{q, a, f, s}`-Schema, gleicher Trainings-/Quizpfad wie Naturwissenschaften. Die Themen folgen Rahmenlehrplan/KLP Mathematik SI und decken Rechnen, Algebra, Funktionen, Geometrie, Statistik/Wahrscheinlichkeit und Exponential-/Trigonometrie-Grundlagen altersgerecht ab. Antworten sind kurze Strings; Brueche werden als `a/b`, Dezimalzahlen mit Punkt oder Komma akzeptiert.

**Geschichts-Pools (Klasse 5–10, seit v71; auf 200 je Klasse seit v73):** Pro Klassenstufe ein 200er Pool mit `{q, a, f, s}`-Schema. Themen folgen dem **NRW-Kernlehrplan Geschichte Sekundarstufe I** (Realschule/Gesamtschule/Gymnasium G9 in der jeweils aktuellen Fassung): Klasse 5 Steinzeit/Aegypten/Antike, Klasse 6 Mittelalter, Klasse 7 Reformation/Absolutismus/Aufklaerung, Klasse 8 Revolution/Industrialisierung/Reichsgruendung, Klasse 9 Kaiserreich/WK1/Weimar/NS, Klasse 10 WK2/Kalter Krieg/Wiedervereinigung/Globalisierung. Antworten sind kurze Strings (Jahreszahl, Ort, Akteur, Schluesselbegriff) und werden ueber `normalize()` verglichen; Umlaute durch `ae/oe/ue/ss` ersetzt.
...
**Grundschul-Faecher (Klasse 1–4, seit v88; fachlich zweitgeprueft seit v88c):** Quellbasis ist der **NRW-Lehrplan Grundschule** (Stand 2021) — Mathematik (alle Klassen), Deutsch (alle Klassen), Sachunterricht (alle Klassen), Englisch (ab Klasse 3). Mathematik Klasse 1/2 bleibt generatorbasiert (`mode: 'generated'`) und liefert jeweils deutlich mehr als 50 unterscheidbare Aufgaben im Zahlenraum des Lehrplans. Alle uebrigen aktiven Grundschul-Faecher sowie Mathematik Klasse 3/4 sind `pool`-Eintraege im `{q, a}`-Schema und enthalten seit v88c jeweils mindestens 100 Items. Der Validator prueft seit v88c fuer alle pool-basierten K1-K4-Faecher mindestens 50 Items; Generator-Faecher werden fachlich ueber den Generatorbereich geprueft. **Bewusst NICHT** umgesetzt (mit Begruendung im Datei-Header `js/data/schueler.js`): Religion/Ethik (konfessionell/kontrovers, nicht plattformneutral), Musik (Audio-Wiedergabe), Kunst (visuell-praktisch), Sport (motorisch) — nicht sinnvoll als Text-Quiz vermittelbar.
...
- **Neue Aufgabe für Klasse 1–4 hinzufügen** (Grundschul-Faecher Deutsch/Sachunterricht/Englisch sowie Mathe-Pools Klasse 3/4): an das passende `pool_kN_fach`-Array bzw. `pool_klasseN_mathe()` in `js/data/schueler.js` anhängen (Reihenfolge irrelevant — die UI sampelt zufällig). Mathematik Klasse 1/2 wird ueber `gen_klasse1_mathe()` bzw. `gen_klasse2_mathe()` erweitert. Schema `{q, a}` Plain-Text; Antwort-Vergleich via `SCH.normalize()` (Umlaut-Folding ae/oe/ue/ss + Akzent-Tolerance kommt aus `schueler_200_topups.js`).
...
- **Quiz**: 10 zufällig gezogene Fragen aus dem Pool des Kapitels; Multi-Choice mit Radio-Buttons (4 Optionen); **Sofort-Feedback nach jeder Antwort** (P-UI-QUIZ-INSTANT-FEEDBACK, seit v111): korrekte Option wird gruen markiert, die falsch gewaehlte rot, die `explanation` inklusive Quellenanker erscheint sofort. Optionen sind danach gesperrt; weiter geht es ueber `Naechste Frage` bzw. `Auswertung anzeigen`. Gilt auch fuer den Wiederholungs-Modus (SRS), **nicht** fuer den Pruefungsmodus (§18.10).

...
- **Status:** ready
- **Dateien:** `AGENTS.md` (§5/§18), `js/app.jsx`, exemplarisch ein Trainings- und ein Schulungs-Datenskript
...
> **Naechster Vorschlag (vom Agent gesetzt):** **P-ARCH-ITEM-SCHEMA** (Datenfundament fuer die Lernplattform-Reife laut Block G/H/I).
> Begruendung: Mit dem nun verabschiedeten Modulhandbuch (`docs/CURRICULUM-MATRIX.md`) ist klar, welche Kapitel/Module in allen drei Studiengaengen mit welchem Lernziel und welcher Taxonomie zu liefern sind. Bevor weitere Inhaltspakete (P-CYBERSEC-07/08/09, P-AUTO-05/06/07/08/09, Medizin-Fachkapitel) gefuellt werden, sollte das Quiz-Item-Schema um Lernziel-Tag, Taxonomie-Level und Quellenanker als Feld erweitert werden — sonst muss spaeter mit grossem Aufwand nachgepflegt werden. Direkt danach folgen **P-STUDY-DIDACTIC-TEMPLATE** (Lehrseiten-Konvention) und **P-CYBERSEC-01-TOPUP** (erstes inhaltliches Top-up unter neuem Schema).

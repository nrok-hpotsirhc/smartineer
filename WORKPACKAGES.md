...
> **Naechster Batch-Anker (vom Agent gesetzt):** **P-ARCH-GLOSSARY** (eigenstaendiges Schulungs-Glossar mit Quiz-/Reader-Verlinkung) oder **P-ARCH-APPJSX-SPLIT** (Code-Hygiene; `js/app.jsx` ueber 3000 Zeilen). Alternativ Inhalts-Top-up wie **P-CYBERSEC-09** oder ein Quellenaudit-Block.
> Begruendung: Mit v57 ist P-LP-NOTES-BOOKMARKS live — Reader-Toolbar trennt Bookmark-Toggle und Notiz-Panel, Persistenz haengt am `useReaderAnnotations()`-Hook, Export/Import deckt beide neuen Keys ab, AGENTS §19 dokumentiert das Merge-Verhalten. Damit ist Block H "Lernplattform-Mechanik" bis auf P-LP-MASTERY (Bloom-Aggregation) abgearbeitet. Naechste Hebel: Block G (Glossar, App-Split) oder Inhaltsausbau auf den noch dünnen Capstone-Kapiteln (Cybersec Kap.10, Automation Kap.7).

...
> **Naechster Batch-Anker (vom Agent gesetzt):** **P-ARCH-GLOSSARY** (eigenstaendiges Schulungs-Glossar mit Quiz-/Reader-Verlinkung) oder **P-LP-NOTES-BOOKMARKS** (Reader-Toolbar mit Notiz/Bookmark, Export/Import-Anbindung). Alternativ: **P-ARCH-APPJSX-SPLIT** (Code-Hygiene; `js/app.jsx` waechst weiter) oder ein **P-CYBERSEC-09** Top-up.
> Begruendung: Mit v56 ist die Hint-Leiter (P-LP-HINT-LADDER) live — `TaskView` rendert progressive `h1/h2/h3 -> h`-Stufen, Schema in AGENTS §5 dokumentiert, Demo am Control-IMC-L3-Item. Damit ist der Block H "Lernplattform-Mechanik" bis auf P-LP-NOTES-BOOKMARKS abgearbeitet. Naechste Hebel sind entweder Glossar/Notizen (Block G/H Restpaketen) oder die seit Monaten aufgeschobene App-jsx-Split (~3000 Zeilen).

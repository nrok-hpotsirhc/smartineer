...
> **Naechster Batch-Anker (vom Agent gesetzt):** **P-LP-FEEDBACK-LINKS** — jetzt unblockt durch P-ARCH-LO-COMPETENCE: Auf dem Quiz-Result-Screen pro falscher Frage Mini-Karten "Lehrseite X.Y nachlesen", gematcht ueber `lo`/`tags` <-> `page.lo`/`page.tags`. Direkter Folgesprung aus der bestehenden Heatmap. Alternativ-Anker: **P-ARCH-ASSESSMENT-ENGINE** (entblockt jetzt P-LP-EXAM-MODE und P-ARCH-CROSS-CHAPTER-EXAM) oder **P-LP-INLINE-CHECK** (jetzt unblockt, Beispielkapitel reicht).
> Begruendung: Mit v51 sind LOs und Tags an einem produktiven Beispielkapitel (Cybersec Kap. 5) plus Heatmap-Renderer im Quiz-Result vorhanden. Direkter naechster Hebel: Lehrseiten-Verlinkung auf Lernziele/Tags, damit aus "rot" sofort die richtige Repeat-Stelle wird.

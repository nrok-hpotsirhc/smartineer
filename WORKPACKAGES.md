...
> **Naechster Vorschlag (vom Agent gesetzt):** **P-ARCH-STABLE-QID** (Stabile Quiz-Item-Identitaet via content-Hash).
> Begruendung: Mit `toItem()` und dem einheitlichen Schema (siehe AGENTS §22) ist die Brücke gebaut. Bevor SRS/Mastery (P-LP-SRS-OPEN, P-LP-MASTERY) wirklich kapitelübergreifend tragen, muss das `id`-Feld stabil sein (Index-Drift-Risiko gemaess AGENTS §11/§18.3). Direkt danach folgt **P-STUDY-DIDACTIC-TEMPLATE** (Lehrseiten-Konvention) und im Inhaltsausbau **P-CYBERSEC-01-TOPUP** (erstes inhaltliches Top-up unter neuem Schema).
